...
class GenomicArray(object):
    ...
    def merge_similar(self, method):
        """"Merge bins according to method"""
        # f = open("selfdata.txt", "w")
        # f.write(str(self.data))
        # f.close()

        # logging.info(str(self.data.columns))
        tmp = pd.DataFrame(columns=self.data.columns)
        for i in range(0, len(method)):
            ...
            for i in range(start, end + 1):
                depth_sum += self.data.loc[i]['depth'] * (self.data.loc[i]['end'] - self.data.loc[i]['start'])
                gc_sum += self.data.loc[i]['gc'] * (self.data.loc[i]['end'] - self.data.loc[i]['start'])
                spread_sum += self.data.loc[i]['spread'] * (self.data.loc[i]['end'] - self.data.loc[i]['start'])
                # span_sum += table.loc[i]['end'] - table.loc[i]['start']
            new_depth = depth_sum / (self.data.loc[end]['end'] - self.data.loc[start]['start'])
            new_gc = gc_sum / (self.data.loc[end]['end'] - self.data.loc[start]['start'])
            new_spread = spread_sum / (self.data.loc[end]['end'] - self.data.loc[start]['start'])
            new_log2 = np.log2(new_depth)
            list = pd.DataFrame([[self.data.loc[start]['chromosome'], int(self.data.loc[start]['start']),
                                  int(self.data.loc[end]['end']), '-', new_log2, new_depth, new_gc, 'Nan', new_spread]],
                                columns=tmp.columns)
        ...
        tmp['gc'] = tmp['gc'].astype(np.float)
        tmp['spread'] = tmp['spread'].astype(np.float)
        # logging.info(tmp.dtypes)
        result = self.as_dataframe(tmp)
        return result
