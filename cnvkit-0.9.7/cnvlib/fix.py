...
def do_fix(target_raw, antitarget_raw, reference,
    ...
    # tabio.write(reference, 'debug_cnarr.cnr')
    ...
    #  在这里把copyNumberArray按method进行区间划分
    # reference = merge_reference(reference, method)
    reference = reference.merge_similar(method)
...
def apply_weights(cnarr, ref_matched, log2_key, spread_key, epsilon=1e-4):
    ...
    tgt_var = descriptives.biweight_midvariance(tgt_cna
                                                ...
                                                .residuals()) ** 2
    f = open("debug.txt", "w")
    f.write(str((tgt_cna['end'] - tgt_cna['start'])))
    f.write(str(type(tgt_cna['end'] - tgt_cna['start'])))
    f.close()

    bin_sz = np.sqrt((tgt_cna['end'] - tgt_cna['start']).astype('int'))
