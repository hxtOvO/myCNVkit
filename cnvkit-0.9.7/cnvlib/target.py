...
def do_target(bait_arr, annotate=None, do_short_names=False, do_split=False,
    ...
    if do_split:
        ...
        #  before subdivide  在这里输出一下tgt_arr看看格式和变化
        # logging.info("before subdivide")
        # for i in range(tgt_arr.__len__()):
        #     logging.info(tgt_arr.__getitem__(i))

        tgt_arr = tgt_arr.subdivide(avg_size, 0)  # Split this array's regions into roughly equal-sized sub-regions.

        #  after subdivide
        # logging.info("after subdivide")
        # for i in range(tgt_arr.__len__()):
        #     logging.info(tgt_arr.__getitem__(i))
        #  修改后，基因被分割为了连续的小段，可以在终端观察到结果
